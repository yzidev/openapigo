...
func (r *RouteBuilder) Query(params ...goas.QueryParam) *RouteBuilder {
	r.def.QueryParams = append(r.def.QueryParams, params...)
	return r
}

func (r *RouteBuilder) Headers(params ...goas.HeaderParam) *RouteBuilder {
...
func (r *RouteBuilder) Responses(specs ...goas.ResponseSpec) *RouteBuilder {
...
// Example:
//
//	s.POST("/users/upload").MultipartUpload("file", goas.MultipartField{Name: "note", Type: goas.ParamString}).Res(map[string]string{}).OK()
//
// Under the hood this uses goas.MultipartFile marker so the OpenAPI builder can render
// multipart/form-data with binary file part.
func (r *RouteBuilder) MultipartUpload(fileField string, fields ...goas.MultipartField) *RouteBuilder {
	if fileField == "" {
		fileField = "file"
	}
	r.def.ReqSchema = goas.MultipartSchema(fileField, fields...)
	r.sb.b.spec[r.key] = r.def
	return r
}
